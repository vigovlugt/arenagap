...
export function runQwikJsonDebug(window2, document2, debug) {
    const parseQwikJSON = () => {
        ...
        const rawData = JSON.parse(
            document2.querySelector('script[type="qwik/json"]').textContent
        );
        const derivedFns =
            ((_a22 = document2.querySelector('script[q\\:func="qwik/json"]')) ==
                null
                ? void 0
                : _a22.qFuncs) || [];
    ...
    if (document2.querySelector('script[type="qwik/json"]')) parseQwikJSON();
    else document2.addEventListener("DOMContentLoaded", parseQwikJSON);
}
export function qwikJsonDebug(document2, qwikJson, derivedFns) {
    ...
    function getObject(idx, parent2) {
        if (typeof idx == "string") {
            if (idx.startsWith("#")) {
                const node2 = nodeMap.get(idx.substring(1));
                if (!node2.__backRefs) node2.__backRefs = [];
                if (node2.__backRefs.indexOf(parent2) === -1)
                    node2.__backRefs.push(parent2);
                return node2;
            }
                ...
                switch (rawValue.charCodeAt(0)) {
                    ...
                    case 2:
                        ...
                        obj = new QRL(
                            ...
                            symbol,
                            capture.map((id22) =>
                                getObject(parseNumber(id22), parent2)
                            )
                        );
                        break;
                    case 3:
                        ...
                        obj = new Task2(
                            ...
                            flagString,
                            parseNumber(index2),
                            getObject(objId, parent2)
                        );
                        break;
                    case 18:
                        obj = getObject(data2, parent2);
                        break;
                    case 17:
                        const fnParts = data2.split(" ");
                        const fn =
                            derivedFns[parseNumber(fnParts.pop().substring(1))];
                        obj = new DerivedSignal(
                            idx,
                            fn,
                            fnParts.map((id22) => getObject(id22, parent2))
                        );
                        break;
                    case 5:
                        obj = new URL(data2);
        ...
        if (parent2 && obj && obj.__backRefs.indexOf(parent2) === -1)
            obj.__backRefs.push(parent2);
        return obj;
    }
    function getNodeMap() {
    ...
    function isComment2(node2) {
        return (
            node2 &&
            typeof node2 == "object" &&
            node2.nodeType === Node.COMMENT_NODE
        );
    }
            ...
            if (
                ...
                for (const url of urls) {
                    ...
                    const qrl = new QRL(
                        ...
                        (capture || "")
                            .split(" ")
                            .map((id2) => refMap[parseInt(id2, 10)])
                    );
